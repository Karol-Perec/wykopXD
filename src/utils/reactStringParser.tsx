...
export class ReactStringParser {
  ...
  private static parseNode(node: Node, splitter: RegExp, renderer: Renderer, key: Key): Node[] | Node {
    return typeof node === 'string'
      ? this.parseTextNode(node, splitter, renderer, key)
      : this.parseElementNode(node, splitter, renderer, key);
  }

  private static parseTextNode(text: string, splitter: RegExp, renderer: Renderer, key: Key): Node[] {
    const result = text.split(splitter);
    if (result.length < 2) return result;
    return result.map((n, idx) => (idx % 2 ? renderer(n, `${key}-${idx}`) : n));
  }

  private static parseElementNode(element: ReactElement, splitter: RegExp, renderer: Renderer, key: Key): ReactElement {
